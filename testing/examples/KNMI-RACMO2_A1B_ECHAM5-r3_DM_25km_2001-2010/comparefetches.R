...
fetches <- c(400, 2000, 10000)
fnameaffixes <- c('small', 'medium_f', 'large')
fetchnames <- paste(fnameaffixes, 'fetch :', fetches, 'm')
colours <- c('pink', 'red', '#DC143C') # crimson
result <- list()
result10 <- list()
dm <- list()
typicalyear <- list()

optic10 <- list(opticpar_nband_water = 10,
                opticpar_nband_ice = 10,
                opticpar_nband_snow = 10,
                opticpar_frac_water = rep(1 / 10, times = 10),
                opticpar_frac_ice = rep(1 / 10, times = 10),
                opticpar_frac_snow = rep(1 / 10, times = 10),
                opticpar_extincoef_water = c(2, 2, 2, 2, 2, 2, 2, 2, 2, 2),
                opticpar_extincoef_ice = rep(20, times = 10),
                opticpar_extincoef_snow = rep(20, times = 10)
                )


for (i in 1:length(fetches)){
  fetch <- fetches[i]
  fnameaffix <- fnameaffixes[i]
  fetchname <- fetchnames[i]

  parameters <- c(38, fetch, 0.5,
  ...
  result10[[i]] <- data.frame(ts.input,
                              runflake(ts.input, parameters, init.state, optic10,
                                       simpleoptic = FALSE)
                              )[-initiali, ]
...
legend('topleft', legend = fetchnames, lty = 1, col = colours) 
dev.off()



png()
for (i in 1:30) { plot(result[[1]][[i]], result10[[1]][[i]])}
dev.off()
